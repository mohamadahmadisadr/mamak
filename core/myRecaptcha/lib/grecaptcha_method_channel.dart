...
class MethodChannelGrecaptcha extends GrecaptchaPlatform {
  ...
  @visibleForTesting
  final methodChannel = const MethodChannel('my_recaptcha');
  ...
  @override
  Future<String> init(String siteKey) async {
    // if (!await isAvailable) {
    //   throw const ReCaptchaNotAvailableException();
    // }

    return await methodChannel.invokeMethod("init", {"key": siteKey});
  }
  ...
  @override
  Future<String> verifyWithRecaptcha(String action) async {
    if (!await isAvailable) {
      throw const ReCaptchaNotAvailableException();
    }

    return await methodChannel.invokeMethod("verify", {"action": action});
  }
